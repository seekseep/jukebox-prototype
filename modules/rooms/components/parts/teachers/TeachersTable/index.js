...
export default function TeachersTable ({
  ...
  const columns = useMemo(() => ([
    ...
    table.createGroup({
      ...
      cell       : ({ getValue, row: { original } }) => (
        <Link href={getRoomPath(`/teachers/${original.id}`)} passHref>
  ...
  const selection = useSelection(teachers, rowSelection)
  const selectedTeachers = useMemo(() => selection.map(({ data: teacher }) => teacher), [selection])

  const handleDownloadCalendarsStart = useCallback(() => {
    onDownloadCalendars(selectedTeachers)
  }, [onDownloadCalendars, selectedTeachers])

  const handleEditStart = useCallback(() => {
    onEdit(selectedTeachers)
  }, [onEdit, selectedTeachers])

  const handleDelete = useCallback(() => {
    if (!confirm('講師を削除しますか？')) return
    onDelete(selectedTeachers)
  }, [onDelete, selectedTeachers])
      ...
      {selection.length > 0 && (
        <>
          <div className="flex gap-2 flex-wrap p-2">
            {selection.map(({ index, data: teacher }) => (
              <div className="flex bg-gray-100 border rounded items-streach" key={index}>
                <div className="py-1 px-2 text-sm">{teacher.name}</div>
